...
- **按性格标签找歌手**：自由勾选 **37 个性格/生活方式/兴趣标签**（如"好奇开放 +
  书虫 + 追星族"），系统从问卷中圈出最符合这些特征的人群，用他们的真实偏好
  实时合成流派与歌手推荐；
- **性格小测评**：答 **25 道题**（大五人格每维度 5 题）实时算出你的五维得分，
  用最近邻（KNN）从 1010 名受访者中找出与你最像的 10% 人群（约 101 人），
  用他们的真实打分生成专属推荐，并展示你的性格雷达图。——标签 × 流派亲和度矩阵由 `src/08_tag_recommender.py`
  预计算（每标签取问卷最符合的前 30% 人群，流派亲和度 = 人群均分 − 全体均分），
  标签栏带左右滚动按钮（窄屏/标签多时可见全部标签）。
  测评页的题库、人群基准与 KNN 基础表由 `src/09_quiz_recommender.py` 预计算。
- **🔍 歌手查询**：输入歌手名（支持模糊匹配），查看 TA 的流派、流行度、
  音频特征与全体歌手的对比，以及哪类性格画像最可能喜欢 TA——推荐方向反过来。
  歌手索引由 `src/11_artist_index.py` 预计算。
