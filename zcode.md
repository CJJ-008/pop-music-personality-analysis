...
| D14 | 仪表盘是否部署公网 | ①仅本地 localhost ②部署 Streamlit Community Cloud（免费） | 先按②走完授权与验证，**本人决定推迟部署**：待界面功能更完善后再上线（邮箱已验证、GitHub 授权已打通，届时直接 Create app 即可） | 2026-09-26 | 推迟 |
...
| D20 | 标签推荐功能 | ①26个全标签库(性格10+生活8+兴趣6+背景2) ②仅性格10个 ③扩展35个；页面布局：双页并存/替换/仅标签页 | **① 26 个全标签库 + 双页并存**（新增「按性格标签找歌手」页：选标签→圈定参考人群(前30%)→流派亲和度→歌手；算法为标签×流派亲和度矩阵预计算，沿用相对偏好口径；互斥标签同选会抵消并有提示） | 2026-09-25 | 已完成 |
| D21 | 报告入口进仪表盘 | ①仅 GitHub 在线链接 ②GitHub 链接 + 离线 HTML 下载（nbconvert 导出，随 exe 分发） | **②两者都要**：侧边栏"📖 完整分析报告"区 = 在线渲染链接 + 自包含 HTML 下载按钮；HTML 由 07 脚本自动导出（约 2MB）并随 exe 打包 | 2026-09-25 | 已完成 |
| D23 | 性格小测评功能 | ①10题简版 ②25题（大五每维5题） ③更多 | **②25 题**（本人要求"题量多一些"）：答完实时算五维得分→z标准化→欧氏距离取最像的10%人群(101人)→用其真实偏好推荐；题库/人群基准/KNN基础表由 src/09 预计算入库 | 2026-09-26 | 已完成 |
| D22 | 标签库扩充与标签栏交互 | ①保持26个 ②扩充至37个（影视题材系列/工作狂/数码控/乐观开朗/自我反省/独生子女）；标签栏溢出：滚动按钮 / CSS换行 / 多选下拉 | **②扩充至37个 + 左右滚动按钮**（pills 行溢出被静默截断且鼠标无法滚动；修复时发现 st.html 的 script 不执行——CSS 走 st.html、JS 走 srcdoc 同源 iframe 操作父页面 DOM） | 2026-09-25 | 已完成 |
| D24 | 面向非专业用户的知识点解释 | ①集中在 README/技术栈说明里解释 ②页面内嵌 ❓ 问号弹窗（st.popover）③悬停 tooltip | **② 问号弹窗**（用户提出"每个用到专业知识的地方做一个小交互"）：新建 glossary.py 收录 11 个概念（K-Means/大五人格/z-score/相对偏好/随机森林/R²与训练测试集/留一法/流行度/音频特征/特征重要性/p<0.05），文案与技术栈说明.md 同口径；kb()/head_kb()/kb_row() 三种摆法适配标题旁/指标行下/表单前；六页+侧边栏共 20 处按钮；glossary.py 随 app.py 打包进 exe；浏览器逐页验证 + exe 重新打包复验 | 2026-09-26 | 已完成 |
| D25 | 侧边栏交互形式 | ①radio 圆形单选 ②按钮式导航（st.button 循环 + session_state）③第三方 streamlit-option-menu | **② 按钮式导航**（用户要求"不要勾选形式、用点击形式"）：页面导航与画像选择均改为全宽按钮堆叠，当前项 primary 高亮、其余 secondary；选择状态存 st.session_state，点击后 st.rerun；不引入新依赖；exe 重新打包并复验 | 2026-09-26 | 已完成 |
| D26 | 登录状态展示位置 | ①保留侧边栏 ②右上角顶栏小图标+下拉菜单 ③主区右上角 popover | **② 右上角账号图标**（用户要求移出侧边栏）：JS 注入 Streamlit 顶栏（stToolbar 内、⋮ 菜单前），显示「👤 用户名 ▾」，下拉含登录名与退出登录；退出 = 前端清除「记住登录」cookie（CookieManager 前端写入、非 httpOnly、path=/，故可 JS 清除）后整页刷新回登录页；侧边栏登录区块随之移除；注入脚本幂等防重复；图标位置经布局调试（初版被工具栏挤出视口，改为插入 ⋮ 菜单前） | 2026-09-26 | 已完成 |
...
- [x] 性格标签推荐器（D20）：src/08_tag_recommender.py 预计算 26 标签 × 17 流派亲和度矩阵
      → app.py 双页并存（按画像 / 按标签）；AppTest + exe 浏览器端到端验证均通过
- [x] 标签库扩充至 37 个 + 标签栏左右滚动按钮（D22）
      → 浪漫主义→拉丁+0.43、动作片迷→金属+0.32 等新关联合理；
      → st.html 脚本不执行的坑：JS 改走 srcdoc 同源 iframe；点击 › 后 scrollLeft 0→220 实测生效
- [x] 性格小测评（D23）：src/09 预计算 25 题题库 + 人群基准 + KNN 基础表
      → app.py 第三页（三页并存）；AppTest + exe 浏览器端到端验证通过（25题作答→推荐 6/6）
      → 顺带修复：登录成功那次运行登录表单与仪表盘同屏（登录后触发一次 st.rerun）
- [x] ❓ 名词小帮手（D24）：glossary.py 收录 11 个专业概念的通俗弹窗
      → 六页 + 侧边栏共 20 处 ❓ 按钮；文案与技术栈说明.md 同口径；
      → exe 重新打包（glossary.py 进包）并浏览器复验通过
...
| 交互式仪表盘 | `app.py` + `auth.py` | 三页推荐（画像/37标签/25题测评）+ 登录注册认证 |
| 分析脚本 | `src/01`~`09` | 数据获取 → 清洗 → 聚类 → 分类 → 回归 → 映射 → 报告生成 → 标签亲和度 → 测评基准 |
