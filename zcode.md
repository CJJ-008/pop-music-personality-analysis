...
| D18 | 仪表盘分发方式 | ①仅云部署 ②云部署+PyInstaller 打包 exe ③仅 exe ④换桌面框架重写 | **①先云部署**（Streamlit 的标准分发方式；exe 有体积 150~300MB、杀软误报、凭据不能嵌入包内、每次改码需重打包四个代价，暂不做。若日后需要离线演示再回头评估） | 2026-09-25 | 已定 |
